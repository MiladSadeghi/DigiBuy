...
class navBarLg extends HTMLElement {
  ...
  loadUser = async () => {
    ...
    try {
      ...
      this.showUserBtn.innerHTML = `<a id="login" href="/login" class="yes-login btn btn-primary py-0"><i class="bi bi-person"></i> <i class="bi bi-caret-down-fill"></i><input class="position-absolute user-menu-btn" type="checkbox"></a><div class="rounded position-absolute user-menu d-none border-primary fw-bold"><a href="/profile" target="_blank" class="d-flex p-4 rounded">${this.user.userName} <i class="ms-auto bi bi-chevron-right"></i></a><hr class="m-0">
      <a href="/profile/?tab=orders" target="_blank" class="d-flex px-3 py-3 btm-b"><i class="bi bi-bag me-2"></i>Orders</a><a href="/profile/?tab=liked" target="_blank" class="d-flex px-3 py-3 btm-b"><i class="bi bi-suit-heart me-2"></i></i>Favorites</a><a href="/profile/?tab=comments" target="_blank" class="d-flex px-3 py-3 btm-b"><i class="bi bi-chat-left-text me-2"></i>Comments</a><a href="/" class="log-out-btn d-flex px-3 py-3 rounded-bottom"><i class="bi bi-box-arrow-left me-2"></i>Log Out</a></div></div>`;
